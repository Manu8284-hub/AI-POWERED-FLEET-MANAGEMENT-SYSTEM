...
import { allRouteIds, routeSchedule } from './data/routes.js';
...
function SchedulePanel() {
  ...
  return <article className="panel schedule-panel"><div className="panel-head"><div><p className="eyebrow">Route data source</p><h2>2026-27 bus schedule</h2></div><a className="text-button" href={routeSchedule.source} target="_blank" rel="noreferrer">Open PDF ↗</a></div><p className="schedule-intro"><b>{routeSchedule.totalPublishedRoutes} published routes</b> to {routeSchedule.destination}.</p><label className="route-picker">Route <select value={routeId} onChange={(event) => setRouteId(event.target.value)}>{allRouteIds.map((id) => <option key={id} value={id}>Route {id}</option>)}</select></label>{route ? <><div className="route-details"><div><span>First stop</span><b>{route.firstStop}</b></div><div><span>Departure</span><b>{route.departure}</b></div><div><span>Stops</span><b>{route.stops}</b></div><div><span>Campus arrival</span><b>{route.arrival}</b></div></div><p className="via"><span>Via</span>{route.via}</p></> : <p className="route-pdf-note">Route {routeId} is listed in the official schedule. Its full published stop sequence and timings appear in the schedule below.</p>}<div className="schedule-document"><div><span className="eyebrow">Official schedule</span><button className="text-button" onClick={() => setShowSchedule(!showSchedule)}>{showSchedule ? 'Hide route information' : 'Show route information'}</button></div>{showSchedule && <iframe title="Bus routes 2026-27: complete stops and timings" src={`${routeSchedule.source}#page=1&zoom=page-width`} />}</div></article>;
}
